...
# 修改 os-release
sed -i 's|OPENWRT_RELEASE="%D %V %C"|OPENWRT_RELEASE="GWJ %V %C"|' package/base-files/files/usr/lib/os-release

# 修改 luci 界面名称
sed -i "s|DISTRIB_DESCRIPTION='%D %V %C'|DISTRIB_DESCRIPTION='GWJ %V %C'|" package/base-files/files/etc/openwrt_release
...
s|set system.@system\[-1\].hostname=.OpenWrt.|set system.@system[-1].hostname='\''Main'\''|
